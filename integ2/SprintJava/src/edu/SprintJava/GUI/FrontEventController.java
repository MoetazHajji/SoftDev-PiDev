...
public class FrontEventController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       // listViewRestau.setItems(refresh);
        ...
        for (Evenement p3 :  sp.listerEvent2())  {
...
//                Evenement.setIdeee(p3.getId_event());
...
//                });
                 ...
                 final Button btnreservation = new Button("Participer");    
               ...
               //Resteau.setIdd(p3.getReference());
                Ticket r =new Ticket ();
                    System.out.println(p3.getId_event());
                    Ticket.setId_evv(p3.getId_event());
                     
             Ticket.setId_usss(Session.getUser().getId());
             Ticket.setNom_event(p3.getNom_event());
                    System.out.println(p3.getNom_event());
              

            URL fxURL = getClass().getResource("ParticiperEvenement.fxml");
            Parent root = FXMLLoader.load(fxURL);
            Scene scene = new Scene(root);
            Stage stage = new Stage();
            stage.setScene(scene);
...
//                pane2.getChildren().add(btnCommenter);
              ...
              /*  A = Hotel.filename;
                ...
                Label prix = new Label(String.valueOf(p3.getDate_debut()));
                Text AdressT = new Text("adress : ");
                Label adress = new Label(String.valueOf(p3.getDate_fin()));  
                Text payT = new Text("Pays : ");
                Label pay = new Label(p3.getEmplacement());
                 Text telT = new Text(" Tel : ");
                Label tey = new Label(p3.getDescription());
